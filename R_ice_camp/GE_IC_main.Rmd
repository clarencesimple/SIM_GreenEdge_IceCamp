...
ps_TS_merge <- merge_samples(ps_TS,"substrate")

rarefied_curve <- rarecurve(data.frame(otu_table(ps_TS_merge)), step=100, cex=1.5, xlab="No. of reads", ylab="No. of ASVs")
```
...
# how much reads are from ASVs that are unallocated
sum(unallocated$n_reads)/sum(long_biogeo$n_reads)
# count how many ASVs were unallocated
taxa_unallocated <- long %>% filter(biogeo=="Unallocated") 
length(unique(taxa_unallocated$asv_code))
...
#count classes in ice and water
unique(long_phyto_ice$class)
unique(long_phyto_water$class)
unique(long$genus)
...
indic_substrate_plot <- lollipop_chart(indic_substrate_top,asv_code,total_reads,facet=substrate, color=division, pch=biogeo,line_size=0.5, point_size=7) +
  ...
  geom_text(aes(x=asv_code,y=1500,label=PR2_matches),size=8,vjust=-0.4,show.legend = FALSE) +
...
# List of ASVs with 100% match to cultures
blast_supplementary <- blast_supplementary %>% 
  filter(asv_code %in% phyto_taxa$asv_code)

write.csv(blast_supplementary, "../supplementary_tables/photosynthetic_ASVs_cultureMatch.csv")
